...
describe('Initial tests',  () => {
   ...
   /*  it('Direct message access after logging', async () => {
         const login_res = await agent.post('/auth/login').send({username:'piyush',password:'piyush'});
    ...
    //     //Go to message now
       ...
       const message_ress = await agent.get('/close');
        expect(message_ress).to.have.status(200);
